...
public class PatientsController implements Initializable {
    ...
    @FXML
    public ChoiceBox<String> reportsBox;
    @FXML
    public ListView<Patient<Integer>> patientsreportsListView;
    @FXML
    public ListView<Appointments<Integer>> appointmentsreportsListView;

    @FXML
    private TextField getIdReport;
    ...
    void populateReportBox()
    {
        List<String> reports = new ArrayList<>();
        reports.add("1 : See all the appointments of a patient:");
        reports.add("2 : See all the patients with the same illness:");
        reports.add("3 : See the patient booked for an appointment:");
        reports.add("4 : See all the appointments for the patients with the same illness:");
        reports.add("5 : See all the appointments on a certain date:");
        ObservableList<String> repoList = FXCollections.observableArrayList(reports);
        reportsBox.setItems(repoList);
    }
    ...
    public void doReport(javafx.scene.input.MouseEvent mouseEvent) {
        switch (reportsBox.getValue().charAt(0)){
            case '1': { // all appointments for a patient
                Integer id = Integer.parseInt(getIdReport.getText());
                ObservableList<Appointments<Integer>> llist =FXCollections.observableArrayList(dbserv.getAppointmentsofPatient(id));
                appointmentsreportsListView.setItems(llist);
            }
            break;
            case '2':{ // patients with the same illness
                String illness = getIdReport.getText();
                ObservableList<Patient<Integer>> llist = FXCollections.observableArrayList(dbserv.getPatientsWithSameIllness(illness));
                patientsreportsListView.setItems(llist);
            }
            break;
            case '3':{ // patient booked for an appointment
                Integer id = Integer.parseInt(getIdReport.getText());
                patientsreportsListView.setItems(FXCollections.observableArrayList(dbserv.getBookedPersonsForAppointment(id)));
            }
            break;
            case'4': {//all the appointments for the patients with the same illness
                String illness = getIdReport.getText();
                ObservableList<Appointments<Integer>> llist = FXCollections.observableArrayList(dbserv.getAppointmentsForPatientsWithIllness(illness));
                appointmentsreportsListView.setItems(llist);
            }
            break;
            case '5': //all the appointments on a date
            {
                String date = getIdReport.getText();
                ObservableList<Appointments<Integer>> llist = FXCollections.observableArrayList(dbserv.getAppointmentsForPatientsWithIllness(date));
                appointmentsreportsListView.setItems(llist);
            }
            break;
        }
